import { FinalReceiptSplit, Item, ProposedSplits, Receipt, SplitwiseMember } from "./types";
...
const splitCost = (cost: number, members: SplitwiseMember[]) : Split[] => {
  ...
  const splits = members.map(member => ({"memberId" : member.id, "cost": roundedSplit}))

  let i = 0;
  while (remainder > 0.009) { // Use 0.009 to account for floating-point arithmetic issues
...
const splitAndUpdateCost = (costType: 'tip' | 'serviceFee' | 'tax', totalCost: number, involvedMembers: SplitwiseMember[], finalReceiptSplitObject: FinalReceiptSplit) => {
  ...
  costSplit.forEach(memberSplit => {
    ...
    const index = finalReceiptSplitObject.members.findIndex(
      finalMemberSplit => finalMemberSplit.memberInfo.id === memberSplit.memberId
    );
...
const initializeFinalSplit = (members: SplitwiseMember[]) : FinalReceiptSplit => {
...
export const splitReceipt = (receipt: Receipt, proposedSplits : ProposedSplits, members: SplitwiseMember[]) : FinalReceiptSplit => {
  // Initialize split
  ...
  Object.keys(proposedSplits).forEach((itemName) => {
    // Get cost of item
    const itemCost = receipt.items.filter(item => item.name == itemName)[0].totalItemPrice;
    ...
    itemSplit.forEach(memberSplit => {
      ...
      // Update 
      finalReceiptSplitObject.members[index].splits[itemName] = memberSplit.cost;
